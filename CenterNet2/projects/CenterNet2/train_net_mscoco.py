...
def setup(args):
    ...
    cfg.DATASETS.TRAIN = ("mscoco_train",)
    cfg.DATASETS.TEST = ()
    ...
    cfg.SOLVER.MAX_ITER = 506  # 300 iterations seems good enough for this toy dataset; you will need to train longer for a practical dataset
    ...
    cfg.OUTPUT_DIR=cfg.OUTPUT_DIR+"/MSCOCO/debug"
...
def get_coco_dicts(img_dir, annotaion_dir):
    json_file = os.path.join(annotaion_dir, "coco_detectron2_val.json")
...
def registrar_dataset():
    for d in ["train"]:
        ...
        DatasetCatalog.register("mscoco_" + d, lambda d=d: get_coco_dicts("/home/ubq3/bishalworkspace/Overhead_Imagery_survey/Datasets/MSCOCO/images/val2017", "/home/ubq3/bishalworkspace/Overhead_Imagery_survey/Datasets/MSCOCO/labels"))
        MetadataCatalog.get("mscoco_" + d).set(thing_classes=['dog', 'potted plant', 'tv', 'bird', 'cat', 'horse', 'sheep', 'cow', 'bottle', 'couch', 'chair', 'dining table', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'boat', 'person', 'stop sign', 'umbrella', 'tie', 'sandwich', 'bed', 'cell phone', 'refrigerator', 'clock', 'toothbrush', 'truck', 'traffic light', 'fire hydrant', 'parking meter', 'bench', 'frisbee', 'skis', 'snowboard', 'skateboard', 'surfboard', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple', 'orange', 'broccoli', 'carrot', 'pizza', 'cake', 'toilet', 'laptop', 'mouse', 'remote', 'keyboard', 'microwave', 'oven', 'toaster', 'sink', 'book', 'vase', 'scissors', 'teddy bear', 'hair drier', 'backpack', 'handbag', 'suitcase', 'elephant', 'zebra', 'kite', 'hot dog', 'donut', 'giraffe', 'baseball bat', 'baseball glove', 'sports ball', 'bear', 'tennis racket'])
