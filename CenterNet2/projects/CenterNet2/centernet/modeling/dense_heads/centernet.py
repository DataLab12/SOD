...
class CenterNet(nn.Module):
    ...
    def forward(self, images, features_dict, name, gt_instances):
        ...
        for idx, level in enumerate(agn_hm_pred_per_level):
            #print("Per level images: ",len(level))
            for f_name, img in zip(name,level):
                hmap_dict={}
                shape=img.size()[1:]
                img= img.view(shape)
                #print("level shape: ",img.shape)
                #print("file name: ", f_name)
                hmap_dict['hmap']=img.detach().cpu().numpy()
                hmap_dict['name']= f_name + "_" +self.in_features[idx]
                #print("file name: ", hmap_dict['name'])
                outputs.append(hmap_dict)

        # print("Length of all heatmaps: ",len(outputs))

        # print(outputs)

        # for feature_map in outputs:
        #     #print(feature_map.size()[1:])
        #     shape=feature_map.size()[1:]
        #     feature_map=feature_map.view(shape)
        ...
        self.fig = plt.figure(figsize=(100, 100), dpi= 200)

        for i in range(len(outputs)):
            hmap=outputs[i]
            a = self.fig.add_subplot(2, 5, i+1)
            imgplot = plt.imshow(hmap['hmap'])
            a.axis("off")
            a.set_title(hmap['name'], fontsize=30)
        ...
        plt.savefig(str('/home/ubq3/bishalworkspace/Overhead_Imagery_survey/Experiments/Heatmap_analysis/DOTA/Centernet2_CSP_COCO_trained_new/hmap_{}.jpg').format(str(self.cnt)+var), bbox_inches='tight')
        self.cnt+=1
        plt.close()
